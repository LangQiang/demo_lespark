package com.example.lq.myapplication.diffusion;
...
public class DiffusionAnimator {
    ...
    public static final int BEHAVIOR_CENTER_LINE = 5;
    //右侧扩散
    public static final int BEHAVIOR_RIGHT = 6;
    //向左扩散
    public static final int BEHAVIOP_LEFT = 7;
    ...
    private DiffusionAnimator(ViewGroup container, int behavior, int count , int viewWidth, int startTop, int space , int[] srcId) {
        this(container,behavior,count,viewWidth,startTop,space,srcId,null);
    }
    private DiffusionAnimator(final ViewGroup container, final int behavior, int count, int viewWidth, int startTop, int space, int[] srcId, CustomInitViewPosition customInitViewPosition) {
        this.container = container;
        ...
        for (int i = 0; i < count; i++) {
            ...
            RelativeLayout.LayoutParams  params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            params.addRule(RelativeLayout.CENTER_IN_PARENT);
            relativeLayout.addView(particleAnimBean.imageView);
            list.add(particleAnimBean);
        }
        relativeLayout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            @Override
...
//            final int width = relativeLayout.getWidth();
            int top = UIHelper.dip2px((int)(Math.random() * 20 + 10));
            int left = -particleAnimBean.imageView.getWidth();
            ...
            particleAnimBean.imageView.setAlpha((float)(Math.random()));
            final Point end = new Point(mViewWidth, UIHelper.dip2px((int)((-Math.random() * 30) + 40)));
            particleAnimBean.endPoint = end;
            if (mViewWidth != 0) {
                Point controllPoint = new Point(random.nextInt(mViewWidth), random.nextInt(UIHelper.dip2px(density)) - random.nextInt(UIHelper.dip2px(density)));
                BezierEvaluator bezierEvaluator = new BezierEvaluator(controllPoint, BEHAVIOR_RIGHT);
...
//                        particleAnimBean.imageView.setScaleX((1- (Float) animation.getAnimatedValue()) * 1.5f);
//                        particleAnimBean.imageView.setScaleY((1- (Float) animation.getAnimatedValue()) * 1.5f);
//                        particleAnimBean.imageView.setRotation((1- (Float) animation.getAnimatedValue() * 180));
//                        particleAnimBean.imageView.setAlpha(1 - (Float) animation.getAnimatedValue());
                ...
                valueAnimator.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        particleAnimBean.imageView.setVisibility(View.GONE);
                    }
                });
            }
        }
    ...
    private void left(){
        for (final ParticleAnimBean particleAnimBean : list) {
            ...
            final int width = relativeLayout.getWidth();
            int top = mStartTop;
            int left = particleAnimBean.imageView.getWidth() + width;
            final float y = particleAnimBean.imageView.getY();
            final Point start = new Point(top,
                    left);
            particleAnimBean.startPoint = start;
            particleAnimBean.imageView.setX(start.x);
            particleAnimBean.imageView.setY(start.y);
            final Point end = new Point(width,top + random.nextInt(UIHelper.dip2px(density)) - UIHelper.dip2px(density) / 2);
            particleAnimBean.endPoint = end;
            Point controllPoint = new Point(random.nextInt(width),random.nextInt(UIHelper.dip2px(density)));
            BezierEvaluator bezierEvaluator = new BezierEvaluator(controllPoint,BEHAVIOR_RIGHT);
            particleAnimBean.bezierEvaluator = bezierEvaluator;
            final ValueAnimator valueAnimator = ValueAnimator.ofFloat(0,1);
            particleAnimBean.valueAnimator = valueAnimator;
    ...
    private void centerLine() {
        for (final ParticleAnimBean particleAnimBean : list) {
            ...
            particleAnimBean.imageView.setX(start.x);
            particleAnimBean.imageView.setY(start.y);
            final Point end = new Point(random.nextInt(2 * width) - width / 2,random.nextInt(2 * height) - height / 2);
            particleAnimBean.endPoint = end;
...
//            particleAnimBean.valueAnimator.removeAllListeners();
    ...
    public static DiffusionAnimator ofPosition(ViewGroup container, int behavior, int count, int viewWidth, int startTop, int space, int[] srcId){
        DiffusionAnimator diffusionAnimator = new DiffusionAnimator(container,behavior,count,viewWidth,startTop,space,srcId);
        return diffusionAnimator;
    }
    ...
    public void start() {
        if (isReady) {
            ...
            if (currentBehavior == BEHAVIOR_CENTER) {
                ...
                startRise();
            } else if (currentBehavior == BEHAVIOR_CENTER_LINE) {
                startCenterLine();
            }else if (currentBehavior == BEHAVIOR_RIGHT){
                startRight();
            }else if (currentBehavior == BEHAVIOP_LEFT){
                startLeft();
            }
        } else {
            final Timer timer = new Timer();
    ...
    public static class BezierEvaluator implements TypeEvaluator<Point> {
        ...
        public Point evaluate(float t, Point startValue, Point endValue) {
            ...
            if (behavior != DiffusionAnimator.BEHAVIOR_CUSTOM) {
                if (behavior == BEHAVIOR_RIGHT){
                    int x = (int) (t * t * startValue.x + 2 * t * (1 - t) * controllPoint.x + (t) * (t) * endValue.x);
