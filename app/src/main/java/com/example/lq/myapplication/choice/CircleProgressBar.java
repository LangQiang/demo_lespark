package com.example.lq.myapplication.choice;


import android.content.Context;
import android.content.res.TypedArray;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.Paint;
import android.graphics.Paint.Style;
import android.graphics.RectF;
import android.util.AttributeSet;
import android.view.View;

import com.example.lq.myapplication.R;


/**
 * Created by lgong on 2015/1/22.
 */
public class CircleProgressBar extends View {
    private int maxProgress = 100;
    private int progress = 0;
    private int progressStrokeWidth = 4;
    private int circleColor = Color.WHITE;
    private boolean showPercentText = false;
    //画圆所在的距形区域
    RectF oval;
    Paint paint;
    private int paintColor = Color.WHITE;

    public CircleProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        oval = new RectF();
        paint = new Paint();
        paint.setAntiAlias(true);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CircleProgressBar);
        circleColor = typedArray.getColor(R.styleable.CircleProgressBar_Color, Color.parseColor("#19a8e5"));
        paint.setColor(circleColor);
        typedArray.recycle();

    }



    @Override
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int width = this.getWidth();
        int height = this.getHeight();

        if (width != height) {
            int min = Math.min(width, height);
            width = min;
            height = min;
        }
        paint.setColor(paintColor); // 设置画笔颜色
        canvas.drawColor(Color.TRANSPARENT); // 白色背景
        paint.setStrokeWidth(progressStrokeWidth); //线宽
        paint.setStyle(Style.STROKE);

        oval.left = progressStrokeWidth / 2.0f; // 左上角x
        oval.top = progressStrokeWidth / 2.0f; // 左上角y
        oval.right = width - progressStrokeWidth / 2.0f; // 左下角x
        oval.bottom = height - progressStrokeWidth / 2.0f; // 右下角y
        canvas.drawArc(oval, -90, 360, false, paint); // 绘制白色圆圈，即进度条背景
        paint.setColor(circleColor);
        canvas.drawArc(oval, -90, ((float) progress / maxProgress) * 360, false, paint); // 绘制进度圆弧，这里是蓝色
        if (showPercentText) {
            paint.setStrokeWidth(1);
            String text = progress + "%";
            int textHeight = height / 4;
            paint.setTextSize(textHeight);
            int textWidth = (int) paint.measureText(text, 0, text.length());
            paint.setStyle(Style.FILL);
            canvas.drawText(text, width / 2 - textWidth / 2, height / 2 + textHeight / 2, paint);
        }

    }

    public boolean isShowPercentText() {
        return showPercentText;
    }

    public void setShowPercentText(boolean showPercentText) {
        this.showPercentText = showPercentText;
    }

    public int getProgressStrokeWidth() {
        return progressStrokeWidth;
    }

    public void setProgressStrokeWidth(int progressStrokeWidth) {
        this.progressStrokeWidth = progressStrokeWidth;
    }

    public int getMaxProgress() {
        return maxProgress;
    }

    public void setMaxProgress(int maxProgress) {
        this.maxProgress = maxProgress;
    }

    public void setProgress(int progress) {
        this.progress = progress;
        this.invalidate();
    }

    public void setPaintColor(int paintColor) {
        this.paintColor = paintColor;
    }

    public void setCircleColor(int circleColor) {
        this.circleColor = circleColor;
    }
    /**
     * 非ＵＩ线程调用
     */
    public void setProgressNotInUiThread(int progress) {
        this.progress = progress;
        this.postInvalidate();
    }

    public void setPaintCap(Paint.Cap cap) {
        if (paint != null) {
            paint.setStrokeCap(cap);
        }
    }
}
